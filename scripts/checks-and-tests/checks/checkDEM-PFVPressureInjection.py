...
if ('PFVFLOW' in features):
										...
										for gg in range(4):
											if gg <= len(flow.getNeighbors(jj)) - 1:
												if flow.getCellPImposed(flow.getNeighbors(jj)[gg]) == True:
													if (-totalflux[flow.getNeighbors(jj)[gg]]
													   ) < initialvol[flow.getNeighbors(jj)[gg]]:
							...
							if kk != 0:
								...
								for xx in range(4):
									if kk != 0:
										if neigh[xx] <= nvoids:
											jj = neigh[xx]
											if jj != neighok[0] and jj != neighok[1] and jj != neighok[
											        2] and jj != neighok[3]:
	...
	def fluxtest():
		...
		if error > toleranceWarning:
			print(
			        ...
			        QinOk, " vs. total water volume flowing inside dry or partially saturated cells", total
			)
			...
			if no == 0:
				...
				if voidvol - total > toleranceWarning:
					print(
					        ...
					        " vs. total water volume flowing inside dry or partially saturated cells", total
					)
	...
	def addPlotData():
		...
		plot.addData(i1=O.iter, t=O.time, Fupper=F33, Fbottom=F22, Q=QinOk, T=total)
