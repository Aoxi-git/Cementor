...
O.engines=[ForceResetter()
           ...
           ,InteractionLoop(
               ...
               [Ip2_FrictMat_FrictMat_FrictPhys(kn = MatchMaker(algo = 'val',val = 1.e5),ks = MatchMaker(algo = 'val',val = 7.e4))], # 1e5 N/m necessary to keep overlap minimal ?
...
def saveThings(epP0,totP):
    ...
    plot.addData(it = O.iter
                 ,fOnWallRel = fOnWallRel
                 ,elE = elP
                 ,kinT = O.energy['kinTrans'],kinR = O.energy['kinRot']
                 ,dampW = dampW
                 ,epP = epP
                 ,fricW = plastW
                 ,maxZb = maxZb
                 ,idBmax = idBmax
                 ,sumE=sumE
                 ,speed = O.speed,tILoop = ILoop.execTime*1.e-3 # now in us
                 ,time = O.time
                 ,uF=unbalancedForce(), zc = avgNumInteractions())
...
epsVal = numpy.array([[0.1,0.5]
                     ,[0.1,1]
                     ,[1,0.5]
                     ,[1.4,1.2]
                     ,[0.4,1.6]
])
rVal = numpy.array([[0.58,1,0.83]
                   ,[0.42,1,0.83]
                   ,[0.42,1,0.83]
                   ,[0.5,0.7,1.]
                   ,[0.4,1.,0.8]
...
rSphCloud = rcar*max( numpy.linalg.norm(rVal[o]) for o in range(rVal.shape[0]) ) # particles of the cloud radii have to be sqrt(sum(radii)^2) apart to be sure they (their circumscribed circle) do not touch the neighbour. And we take the maximum of that sum/norm
baseBox = 0.25
...
for shape in range(5): # establishing the list of 5 possible distField once for all
    ...
    lsSe = levelSetBody("superellipsoid"
                  ,extents=(rx,ry,rz)
                  ,epsilons = (epsE,epsN)
                  ,spacing = 2*min(rx,ry,rz)/prec
                  ,nSurfNodes = 2,nodesPath=2)
...
for sph in sp: # before directly using for the present definition of bodies
    currB = len(O.bodies)
    dimsP = rVal[currB%5]
    O.bodies.append(levelSetBody(center=(sph[0][0],sph[0][1],sph[0][2])
                           ,grid = grids[currB%5],distField = distFields[currB%5]
                           ,nSurfNodes = nN,nodesPath=2
                           ,dynamic=True
    ...
    volS += b.shape.volume()
tEnd = time.time()
print(len(O.bodies),'LS superellipsoids for a total of',nGP,'grid points and',nNtot,'boundary nodes generated in',tEnd-tStart,'s, and',ramFP() - ramFPini,'MB of RAM')
