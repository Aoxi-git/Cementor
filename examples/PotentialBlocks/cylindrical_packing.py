...
r=0.3*rc
for s in sp:
	pb=platonic_solid(material=m, numFaces=random.choice([4,6,8,12]), rc=rc, r=r, R=0.0, center=s[0], mask=1, isBoundary=False, fixed=False) # FIXME: Icosahedra are excluded for now
	pb.state.ori = Quaternion((random.random(),random.random(),random.random()),random.random()) #s[2]
	O.bodies.append(pb)

plates=cylindricalPlates(material=m, radius=0.5, height=1.0, thickness=0.05, numFaces=12, r=0.005, R=0.0, mask=3, isBoundary=False, fixed=True, lid=[False,True])
