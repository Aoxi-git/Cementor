...
pb=potentialblock(material=m, 
a=[  1, -1,  0,  0,  0,  0 ],
b=[  0,  0,  1, -1,  0,  0 ],
c=[  0,  0,  0,  0,  1, -1 ],
d=[ edges[0]/2.-r, edges[0]/2.-r, edges[1]/2.-r, edges[1]/2.-r, edges[2]/2.-r, edges[2]/2.-r ],
r=r, R=0.0, mask=1, isBoundary=False, fixed=False, color=[1,0,0]) # Red cuboid
...
for numFaces in [4,6,8,12,20]:
	pb=platonic_solid(material=m, numFaces=numFaces, rm=0.01, r=r, R=0.0, center=[xPos,0.05*length,0], mask=1, isBoundary=False, fixed=False); O.bodies.append(pb)
	xPos=xPos+rc
...
pb=platonic_solid(material=m, numFaces=numFaces, volume=0.00001, r=r, R=0.0, center=[-7*rc,0,0], mask=1, isBoundary=False, fixed=False); O.bodies.append(pb)	# Define volume
pb=platonic_solid(material=m, numFaces=numFaces,      edge=0.01, r=r, R=0.0, center=[-6*rc,0,0], mask=1, isBoundary=False, fixed=False); O.bodies.append(pb)	# Define edge
pb=platonic_solid(material=m, numFaces=numFaces,        ri=0.01, r=r, R=0.0, center=[-5*rc,0,0], mask=1, isBoundary=False, fixed=False); O.bodies.append(pb)	# Define inradius
pb=platonic_solid(material=m, numFaces=numFaces,        rm=0.01, r=r, R=0.0, center=[-4*rc,0,0], mask=1, isBoundary=False, fixed=False); O.bodies.append(pb)	# Define inradius
pb=platonic_solid(material=m, numFaces=numFaces,        rc=0.01, r=r, R=0.0, center=[-3*rc,0,0], mask=1, isBoundary=False, fixed=False); O.bodies.append(pb)	# Define circumradius

# ------------------------------------------------------------------------------------------------------------------------------------------ #
# potential_utils.prism: Prisms with different numbers of sides
xPos=-8*rc
for numFaces in [3,4,5,6,7,8]:
	pb=prism(material=m, radius1=0.2*rc, radius2=0.2*rc, thickness=0.2*rc, numFaces=numFaces, r=0.0, R=0.0, center=None, color=[-1,-1,-1], mask=1, isBoundary=False, fixed=False)
...
for numFaces in [15,20,25,30,35,40]:
	pb=prism(material=m, radius1=0.2*rc, radius2=0.2*rc, thickness=0.3*rc, numFaces=numFaces, r=0.0, R=0.0, center=None, color=[-1,-1,-1], mask=1, isBoundary=False, fixed=False)
...
for radius2 in [0.05*rc,0.1*rc,0.15*rc,0.2*rc,0.25*rc,0.3*rc]:
	pb=prism(material=m, radius1=0.2*rc, radius2=radius2, thickness=0.5*rc, numFaces=20, r=0.0, R=0.0, center=None, color=[-1,-1,-1], mask=1, isBoundary=False, fixed=False)
